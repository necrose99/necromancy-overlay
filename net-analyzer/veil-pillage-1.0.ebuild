...
DESCRIPTION="Veil-Pillage is a post-exploitation framework and a part of the Veil framework."
...
src_install() {
	...
	python_fix_shebang "${ED}"/usr/$(get_libdir)/${PN}/Veil-Pillage.py

	#use our custom settings
	insinto /etc/veil
	newins "${FILESDIR}"/${PN}-2.23-settings.py settings.py

	dosym /usr/$(get_libdir)/veil-evasion//Veil-Pillage.py /usr/bin/veil-pillage
}
